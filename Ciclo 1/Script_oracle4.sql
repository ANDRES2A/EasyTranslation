...
alter table SABE_REVISOR
   drop constraint FK_SABE_REV_SABE_REVI_IDIOMA;
...
(
   CORREO_RE            VARCHAR2(255)        not null,
   ABREVIACION_ID       VARCHAR2(10)         not null,
   constraint PK_SABE_REVISOR primary key (CORREO_RE, ABREVIACION_ID)
);
...
alter table SABE_REVISOR
   add constraint FK_SABE_REV_SABE_REVI_IDIOMA foreign key (ABREVIACION_ID)
      references IDIOMA (ABREVIACION_ID);
