...
public class IdiomaFacade extends AbstractFacade<Idioma> {
    ...
    public boolean isRevisorCollectionEmpty(Idioma entity) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Long> cq = cb.createQuery(Long.class);
        Root<Idioma> idioma = cq.from(Idioma.class);
        cq.select(cb.literal(1L)).distinct(true).where(cb.equal(idioma, entity), cb.isNotEmpty(idioma.get(Idioma_.revisorCollection)));
        return em.createQuery(cq).getResultList().isEmpty();
    }

    public Collection<Revisor> findRevisorCollection(Idioma entity) {
    ...
    @Override
    public Idioma findWithParents(Idioma entity) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Idioma> cq = cb.createQuery(Idioma.class);
        Root<Idioma> idioma = cq.from(Idioma.class);
        idioma.fetch(Idioma_.revisorCollection, JoinType.LEFT);
        cq.select(idioma).where(cb.equal(idioma, entity));
        try {
